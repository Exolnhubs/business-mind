...
interface Props {
  posting:  boolean
  onPost:   (data: { type: HappeningType; body: string; expires_in_hours: number }) => Promise<boolean>
  onCancel: () => void
}
...
export function PostHappeningForm({ posting, onPost, onCancel }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!body.trim()) return
    const ok = await onPost({ type, body: body.trim(), expires_in_hours: expiry })
    if (ok) { setBody(''); setType('open_invite'); setExpiry(6) }
  }
