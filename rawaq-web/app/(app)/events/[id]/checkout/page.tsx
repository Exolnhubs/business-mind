...
export default async function CheckoutPage({ params, searchParams }: Props) {
  ...
  if (!user) redirect(`/login?next=/events/${id}/checkout${ticket_type_id ? `?ticket_type_id=${ticket_type_id}` : ''}`)

  // Fetch event
  const { data: event } = await supabase
    .from('events')
    .select('id, title, start_at, end_at, venue_name, city, country, cover_image_url, is_free, price, currency, is_cancelled, is_published, capacity, bookings_count')
    .eq('id', id)
    .single()
