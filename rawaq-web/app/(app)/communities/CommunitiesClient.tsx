...
export function CommunitiesClient({
  ...
  useEffect(() => {
    // Skip client fetch when server already provided trending data
  ...
  }, [cacheScopeKey, initialTrending.length])
  ...
  useEffect(() => {
    // If server pre-rendered the initial page and no filters are active yet,
    // skip the mount fetch for anonymous users — the HTML already has the data.
