...
function buildSystemPrompt(organizers: { id: string; name: string }[]): string {
  const organizerBlock = organizers.length > 0
...
interface FetchResult {
  events: unknown[]
  strategy: 'exact' | 'relaxed_date' | 'relaxed_city' | 'relaxed_category' | 'followed_organizers' | 'none'
  description: string
}
...
async function fetchEventsWithFallback(
  ...
  if (params.dateRange) {
    const relaxed = await queryEvents({ ...params, dateRange: null }, organizerIds)
    if (relaxed.length > 0) {
  ...
  if (params.interests && params.interests.length > 0) {
    const relaxed = await queryEvents({ ...params, interests: [] }, organizerIds)
    if (relaxed.length > 0) {
  ...
  return { events: [], strategy: 'none', description: 'No events found matching any criteria.' }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const fetchResult = await fetchEventsWithFallback(searchParams, organizerIds, organizerNames)
    ...
    const contextBlock = fetchResult.strategy === 'none'
