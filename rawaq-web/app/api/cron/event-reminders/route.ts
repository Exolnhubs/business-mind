...
export async function GET(req: NextRequest) {
  try {
    ...
    const h24From = new Date(now.getTime() + (24 * 60 - 5) * 60 * 1000).toISOString()
    const h24To   = new Date(now.getTime() + (24 * 60 + 5) * 60 * 1000).toISOString()
    ...
    return ok({ sent, h1_events: (h1Events ?? []).length, h24_events: (h24Events ?? []).length })
  } catch (err) {
